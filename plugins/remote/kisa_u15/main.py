"""Remote plugin for KISA U-15 orphan file ownership checks."""
...
DEFAULT_FIND_COMMAND = "find {path} -xdev {prune} \\( -nouser -o -nogroup \\) -ls"
DEFAULT_SEARCH_PATHS = ("/etc", "/var", "/home", "/opt", "/root")
...
class OrphanFileOwnershipCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        find_command = str(
            self.context.config.get("find_command") or DEFAULT_FIND_COMMAND
        ).strip()
        if not find_command:
        ...
        self.add_finding(
            vuln_id="KISA-U-15",
            title=f"{self._format_os(os_type)} 소유자 없는 파일 존재",
            severity="High",
            evidence=evidence,
            tags=["KISA:U-15"],
            description="소유자가 존재하지 않는 파일 또는 디렉터리가 발견되었습니다.",
            solution="불필요한 파일을 삭제하거나 적절한 소유자를 부여하세요.",
        )
        return self.results
    ...
    def _build_command(
        ...
        if "{path}" in command:
            ...
            command = self._inject_path(command, path)
        if "{prune}" in command:
            command = command.replace("{prune}", prune_expr)
        elif prune_expr:
            command = f"{command} {prune_expr}"
        return command
    ...
    def _inject_path(self, command: str, path: str) -> str:
        try:
            tokens = shlex.split(command)
        except ValueError:
            return f"find {shlex.quote(path)} {command}"
    ...
    def _add_unavailable(self, os_type: str, command, result: CommandResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-15",
            title=f"{self._format_os(os_type)} 소유자 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-15"],
