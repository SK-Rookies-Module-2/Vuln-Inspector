"""Remote plugin for KISA U-26 non-device file checks in /dev."""
...
DEFAULT_FIND_COMMAND = "find {path} -xdev {prune} -type f -ls"
DEFAULT_SEARCH_PATHS = ("/dev",)
DEFAULT_EXCLUDE_PATHS = ("/dev/shm", "/dev/mqueue")
DEFAULT_WHITELIST_PATHS = ("/dev/MAKEDEV", "/dev/MAKEDEV.local")
...
class NonDeviceFileCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        whitelist_paths = set(
            ...
            if path.strip()
        ) or set(DEFAULT_WHITELIST_PATHS)
        ...
        self.add_finding(
            vuln_id="KISA-U-26",
            title=f"{self._format_os(os_type)} /dev 일반 파일 존재",
            severity="High",
            evidence=evidence,
            tags=["KISA:U-26"],
            description="/dev 디렉터리에 장치 파일이 아닌 일반 파일이 존재합니다.",
            solution="불필요한 일반 파일을 삭제하고 /dev에 장치 파일만 유지하세요.",
        )
        return self.results
    ...
    def _add_unavailable(self, os_type: str, command, result: CommandResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-26",
            title=f"{self._format_os(os_type)} /dev 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-26"],
