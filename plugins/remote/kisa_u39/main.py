"""Remote plugin for KISA U-39 NFS service disable checks."""
...
DEFAULT_PROCESS_PATTERN = "nfsd|statd|lockd"
...
def _find_nfs_processes(lines: List[str], pattern: re.Pattern[str]) -> List[str]:
    matches = []
...
class NfsServiceDisableCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        matches = _find_nfs_processes(result.lines, regex)
        if not matches:
            return self.results
        ...
        self.add_finding(
            vuln_id="KISA-U-39",
            title=f"{self._format_os(os_type)} NFS 서비스 활성화",
            severity="High",
            evidence=evidence,
            tags=["KISA:U-39"],
            description="NFS 관련 데몬이 실행 중입니다.",
            solution="불필요한 NFS 서비스를 중지하고 비활성화하세요.",
        )
        return self.results
    ...
    def _add_unavailable(self, os_type: str, command: str, result: CommandResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-39",
            title=f"{self._format_os(os_type)} NFS 서비스 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-39"],
            description="NFS 서비스 프로세스를 확인할 수 없습니다.",
