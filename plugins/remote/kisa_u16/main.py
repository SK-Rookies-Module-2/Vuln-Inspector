"""Remote plugin for KISA U-16 passwd file permission checks."""
...
class PasswdFilePermissionCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        result = self._read_permission(passwd_path)
        ...
        if issues:
            ...
            self.add_finding(
                vuln_id="KISA-U-16",
                title=f"{self._format_os(os_type)} /etc/passwd 권한 설정 미흡",
                severity="High",
                evidence=evidence,
                tags=["KISA:U-16"],
                description="소유자 또는 권한 설정이 기준을 만족하지 않습니다.",
                solution="소유자를 root로 설정하고 권한을 644 이하로 제한하세요.",
            )
    ...
    def _add_unavailable(self, os_type: str, path: Path, result: PermissionResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-16",
            title=f"{self._format_os(os_type)} /etc/passwd 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-16"],
            description="/etc/passwd 권한 정보를 확인할 수 없습니다.",
