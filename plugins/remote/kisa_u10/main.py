"""Remote plugin for KISA U-10 duplicate UID checks."""
...
def _parse_passwd(lines: List[str]) -> List[PasswdEntry]:
    ...
    for raw_line in lines:
        ...
        try:
            ...
            continue
        entries.append(PasswdEntry(name=name, uid=uid, line=raw_line.strip()))
    return entries


class DuplicateUidCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        exclude_uids = _normalize_int_list(
            self.context.config.get("exclude_uids"),
            "exclude_uids",
        )
        ...
        uid_map: Dict[int, List[PasswdEntry]] = {}
        for entry in entries:
            if entry.uid in exclude_uids:
                continue
            uid_map.setdefault(entry.uid, []).append(entry)

        duplicates = []
        for uid, members in uid_map.items():
            if len(members) > 1:
                duplicates.append(
                    {
                        "uid": uid,
                        "accounts": [member.name for member in members],
        ...
        if duplicates:
            ...
            evidence["detected_value"] = duplicates
            evidence["uid"] = duplicates[0].get("uid")
            evidence["line"] = (duplicates[0].get("lines") or [None])[0]
            self.add_finding(
                vuln_id="KISA-U-10",
                title=f"{self._format_os(os_type)} 동일 UID 존재",
                severity="Medium",
                evidence=evidence,
                tags=["KISA:U-10"],
                description="동일한 UID를 가진 계정이 존재합니다.",
                solution="중복된 UID를 제거하거나 UID를 변경하세요.",
            )
    ...
    def _add_unavailable(self, os_type: str, path: Path, result: ReadResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-10",
            title=f"{self._format_os(os_type)} UID 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-10"],
