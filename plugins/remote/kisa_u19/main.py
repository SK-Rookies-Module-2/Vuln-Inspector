"""Remote plugin for KISA U-19 hosts file permission checks."""
...
class HostsFilePermissionCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        hosts_path = Path(self.context.config.get("hosts_path") or DEFAULT_HOSTS_PATH)
        ...
        result = self._read_permission(hosts_path)
        if result.perm is None or result.owner is None:
            self._add_unavailable(os_type, hosts_path, result)
            return self.results
        ...
        if issues:
            evidence = self._base_evidence(os_type, hosts_path, result)
            ...
            self.add_finding(
                vuln_id="KISA-U-19",
                title=f"{self._format_os(os_type)} /etc/hosts 권한 설정 미흡",
                severity="High",
                evidence=evidence,
                tags=["KISA:U-19"],
                description="소유자 또는 권한 설정이 기준을 만족하지 않습니다.",
                solution="소유자를 root로 설정하고 group/other 쓰기 권한을 제거하세요.",
            )
    ...
    def _add_unavailable(self, os_type: str, path: Path, result: PermissionResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-19",
            title=f"{self._format_os(os_type)} /etc/hosts 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-19"],
            description="/etc/hosts 권한 정보를 확인할 수 없습니다.",
