"""Remote plugin for KISA U-23 SUID/SGID file checks."""
...
class SuidSgidFileCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        self.add_finding(
            vuln_id="KISA-U-23",
            title=f"{self._format_os(os_type)} SUID/SGID 파일 과다",
            severity="High",
            evidence=evidence,
            tags=["KISA:U-23"],
            description="화이트리스트에 없는 SUID/SGID 파일이 발견되었습니다.",
            solution="불필요한 SUID/SGID 파일을 제거하거나 권한을 회수하세요.",
        )
        return self.results
    ...
    def _add_unavailable(self, os_type: str, command, result: CommandResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-23",
            title=f"{self._format_os(os_type)} SUID/SGID 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-23"],
