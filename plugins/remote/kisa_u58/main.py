"""Remote plugin for KISA U-58 SNMP service checks."""
...
class SnmpServiceCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        snmp_process_names = _normalize_list(
            self.context.config.get("snmp_process_names"),
            "snmp_process_names",
        ) or ["snmpd"]
        ...
        matches = _find_processes(result.lines, snmp_process_names)
        if not matches:
            return self.results
        ...
        evidence = {
            ...
            "config_path": {
                ...
                "snmp_process_names": snmp_process_names,
            },
            "mode": result.mode,
            "detected_value": limited,
            "count": len(matches),
            "checked_sources": ["process"],
        }
        if host:
            evidence["host"] = host
        ...
        self.add_finding(
            vuln_id="KISA-U-58",
            title=f"{self._format_os(os_type)} SNMP 서비스 활성화",
            severity="Medium",
            evidence=evidence,
            tags=["KISA:U-58"],
            description="SNMP 서비스가 활성화되어 있습니다.",
            solution="SNMP 서비스를 중지하거나 필요한 경우에만 사용하세요.",
        )
        return self.results
    ...
    def _add_unavailable(self, os_type: str, command: str, result: CommandResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-58",
            title=f"{self._format_os(os_type)} SNMP 서비스 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-58"],
            description="SNMP 서비스 상태를 확인할 수 없습니다.",
