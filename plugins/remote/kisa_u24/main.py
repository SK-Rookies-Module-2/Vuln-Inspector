"""Remote plugin for KISA U-24 user environment file permission checks."""
...
class UserEnvFilePermissionCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        env_file_names = _normalize_list(
            self.context.config.get("env_file_names"),
            "env_file_names",
        ) or list(DEFAULT_ENV_FILES)
        if not env_file_names:
            raise PluginConfigError("env_file_names must include at least one entry")
        ...
        for entry in entries:
            for name in env_file_names:
                ...
                if not name:
                    continue
                path = Path(name) if name.startswith("/") else entry.home / name
                result = self._read_permission(path, client, host)
                if file_mode is None:
                    file_mode = result.mode
                if result.missing:
                    missing_count += 1
                    continue
                ...
                checked_count += 1
                issue_flags = self._evaluate_issues(
                    result.owner,
                    result.perm,
                    entry.name,
                    allow_root_owner,
        ...
        evidence = {
            ...
            "config_path": self._stringify_config_path(
                {"passwd": passwd_path, "env_file_names": env_file_names}
            ),
            "mode": self._merge_modes({"passwd": passwd_result.mode, "files": file_mode}),
        ...
        self.add_finding(
            vuln_id="KISA-U-24",
            title=f"{self._format_os(os_type)} 사용자 환경변수 파일 권한 설정 미흡",
            severity="High",
            evidence=evidence,
            tags=["KISA:U-24"],
            description="환경변수 파일의 소유자 또는 권한 설정이 기준을 만족하지 않습니다.",
            solution="환경변수 파일을 root 또는 해당 계정이 소유하도록 하고 group/other 쓰기 권한을 제거하세요.",
        )
        return self.results
    ...
    def _add_unavailable(
        ...
        evidence = {
            "os_type": os_type,
            "config_path": self._stringify_config_path(
                {"passwd": passwd_path, "env_file_names": list(env_file_names)}
            ),
            "mode": self._merge_modes({"passwd": result.mode, "files": file_mode}),
        }
        if result.host:
        ...
        self.add_finding(
            vuln_id="KISA-U-24",
            title=f"{self._format_os(os_type)} 환경변수 파일 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-24"],
            description="환경변수 파일 권한 정보를 확인할 수 없습니다.",
