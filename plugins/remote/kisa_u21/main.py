"""Remote plugin for KISA U-21 syslog permission checks."""
...
DEFAULT_SYSLOG_PATH = "/etc/syslog.conf"
DEFAULT_RSYSLOG_PATH = "/etc/rsyslog.conf"
...
class SyslogConfigPermissionCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        syslog_path = Path(self.context.config.get("syslog_conf_path") or DEFAULT_SYSLOG_PATH)
        rsyslog_path = Path(self.context.config.get("rsyslog_conf_path") or DEFAULT_RSYSLOG_PATH)
        ...
        allow_other_permissions = bool(self.context.config.get("allow_other_permissions", False))

        checked = []
        errors: List[PermissionResult] = []
        findings = []

        for path in (syslog_path, rsyslog_path):
            ...
            if issues:
                findings.append(self._build_entry(path, result, issues))
        ...
        evidence = {
            ...
            "mode": self._merge_modes({str(idx): item["result"].mode for idx, item in enumerate(findings)}),
            "detected_value": [item["evidence"] for item in findings],
            "count": len(findings),
        }
        host = next((item["result"].host for item in findings if item["result"].host), None)
        if host:
            evidence["host"] = host
        if errors:
        ...
        self.add_finding(
            vuln_id="KISA-U-21",
            title=f"{self._format_os(os_type)} syslog 권한 설정 미흡",
            severity="High",
            evidence=evidence,
            tags=["KISA:U-21"],
            description="syslog/rsyslog 설정 파일의 소유자 또는 권한 설정이 기준을 만족하지 않습니다.",
            solution="소유자를 root/bin/sys로 설정하고 권한을 640 이하로 제한하세요.",
        )
        return self.results
    ...
    def _evaluate_issues(
        ...
        if not allow_other_permissions and result.perm is not None and (result.perm & 0o007):
            issues.append("other_permissions")
        return issues
    ...
    def _add_unavailable(self, os_type: str, path, result: PermissionResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-21",
            title=f"{self._format_os(os_type)} syslog 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-21"],
            description="syslog/rsyslog 설정 파일 권한 정보를 확인할 수 없습니다.",
