"""Remote plugin for KISA U-28 access restriction checks."""

from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
import os
import pwd
import re
import shlex
import stat
import subprocess
from typing import Dict, List, Optional, Sequence, Tuple

from app.adapters.ssh import SshClient
from app.core.errors import AdapterError, PluginConfigError
from app.core.plugin_base import BasePlugin
from app.core.types import Finding

OS_TYPES = {"linux", "solaris", "aix", "hpux"}


@dataclass
class ReadResult:
    lines: Optional[List[str]]
    mode: str
    error: Optional[str] = None
    host: Optional[str] = None
    path: Optional[Path] = None


@dataclass
class PermissionResult:
    perm: Optional[int]
    owner: Optional[str]
    mode: str
    error: Optional[str] = None
    host: Optional[str] = None
    path: Optional[Path] = None
    raw: Optional[str] = None
    method: Optional[str] = None
    missing: bool = False


@dataclass
class CommandResult:
    lines: Optional[List[str]]
    mode: str
    error: Optional[str] = None
    host: Optional[str] = None
    command: Optional[str] = None


def _normalize_list(value, name: str) -> List[str]:
    if value is None:
        return []
    if isinstance(value, str):
        return [value]
    if isinstance(value, list):
        if not all(isinstance(item, str) for item in value):
            raise PluginConfigError(f"{name} must be an array of strings")
        return value
    raise PluginConfigError(f"{name} must be an array of strings")


def _strip_comment(raw_line: str) -> str:
    line = raw_line.strip()
    if not line or line.startswith("#"):
        return ""
    if "#" in line:
        line = line.split("#", 1)[0].rstrip()
    return line


def _parse_stat_output(raw: str) -> Optional[Tuple[int, str]]:
    parts = raw.strip().split()
    if len(parts) < 2:
        return None
    mode_raw, owner = parts[0], parts[1]
    try:
        mode_value = int(mode_raw, 8)
    except ValueError:
        return None
    return mode_value, owner


def _permstr_to_mode(perm: str) -> Optional[int]:
    if len(perm) < 10:
        return None
    perms = perm[1:10]
    mode = 0
    mapping = [
        (0, "r", 0o400),
        (1, "w", 0o200),
        (2, "x", 0o100),
        (3, "r", 0o040),
        (4, "w", 0o020),
        (5, "x", 0o010),
        (6, "r", 0o004),
        (7, "w", 0o002),
        (8, "x", 0o001),
    ]
    for idx, expected, bit in mapping:
        if perms[idx] == expected:
            mode |= bit

    user_exec = perms[2]
    group_exec = perms[5]
    other_exec = perms[8]
    if user_exec in ("s", "S"):
        mode |= stat.S_ISUID
        if user_exec == "s":
            mode |= 0o100
    if group_exec in ("s", "S"):
        mode |= stat.S_ISGID
        if group_exec == "s":
            mode |= 0o010
    if other_exec in ("t", "T"):
        mode |= stat.S_ISVTX
        if other_exec == "t":
            mode |= 0o001

    return mode


def _parse_ls_output(raw: str) -> Optional[Tuple[int, str]]:
    parts = raw.strip().split()
    if len(parts) < 3:
        return None
    permstr = parts[0]
    owner = parts[2]
    mode_value = _permstr_to_mode(permstr)
    if mode_value is None:
        return None
    return mode_value, owner


def _is_missing_error(error: Optional[str]) -> bool:
    if not error:
        return False
    lowered = error.lower()
    return (
        "no such file" in lowered
        or "not found" in lowered
        or "cannot stat" in lowered
        or "cannot access" in lowered
    )


class AccessRestrictionCheck(BasePlugin):
    def check(self) -> List[Finding]:
        target_type = str(self.context.target.get("type") or "").upper()
        if target_type and target_type != "SERVER":
            raise PluginConfigError("Target type must be SERVER for remote checks")

        os_type = str(self.context.config.get("os_type", "")).lower().strip()
        if os_type not in OS_TYPES:
            raise PluginConfigError(f"os_type must be one of {sorted(OS_TYPES)}")

        hosts_deny_path = Path(self.context.config.get("hosts_deny_path") or "/etc/hosts.deny")
        hosts_allow_path = Path(self.context.config.get("hosts_allow_path") or "/etc/hosts.allow")
        required_deny_patterns = _normalize_list(
            self.context.config.get("required_deny_patterns"),
            "required_deny_patterns",
        ) or ["ALL:ALL"]
        require_allow_entries = bool(self.context.config.get("require_allow_entries", True))
        allowed_owners = _normalize_list(
            self.context.config.get("allowed_owners"),
            "allowed_owners",
        ) or ["root"]
        max_mode = self._to_positive_int(self.context.config.get("max_mode", 600), "max_mode")
        allow_group_write = bool(self.context.config.get("allow_group_write", False))
        allow_other_write = bool(self.context.config.get("allow_other_write", False))

        check_iptables = bool(self.context.config.get("check_iptables", True))
        iptables_command = str(self.context.config.get("iptables_command") or "iptables -L -n").strip()
        check_firewalld = bool(self.context.config.get("check_firewalld", True))
        firewalld_command = str(self.context.config.get("firewalld_command") or "firewall-cmd --list-all").strip()
        max_results = self._to_positive_int(self.context.config.get("max_results", 200), "max_results")

        client, host = self._get_ssh_client()

        issues = []
        errors = []
        checked_count = 0
        missing_files = 0

        deny_info = self._inspect_tcp_wrapper_file(
            hosts_deny_path,
            required_deny_patterns,
            allowed_owners,
            max_mode,
            allow_group_write,
            allow_other_write,
            client,
            host,
        )
        if deny_info["status"] == "missing":
            missing_files += 1
        elif deny_info["status"] == "error":
            if deny_info.get("error"):
                errors.append({"path": str(hosts_deny_path), "error": deny_info["error"]})
        else:
            checked_count += 1
            if deny_info.get("issues"):
                issues.append({"file": "hosts.deny", **deny_info})

        allow_info = self._inspect_tcp_wrapper_file(
            hosts_allow_path,
            [],
            allowed_owners,
            max_mode,
            allow_group_write,
            allow_other_write,
            client,
            host,
            require_allow_entries=require_allow_entries,
        )
        if allow_info["status"] == "missing":
            missing_files += 1
        elif allow_info["status"] == "error":
            if allow_info.get("error"):
                errors.append({"path": str(hosts_allow_path), "error": allow_info["error"]})
        else:
            checked_count += 1
            if allow_info.get("issues"):
                issues.append({"file": "hosts.allow", **allow_info})

        firewall_info = self._check_firewall(
            check_iptables,
            iptables_command,
            check_firewalld,
            firewalld_command,
            client,
            host,
        )
        errors.extend(firewall_info.get("errors", []))

        tcp_wrapper_ok = deny_info.get("ok") and allow_info.get("ok")
        firewall_ok = firewall_info.get("ok")

        if tcp_wrapper_ok or firewall_ok:
            return self.results

        if not checked_count and not firewall_info.get("checked") and errors:
            self._add_unavailable(
                os_type,
                {
                    "hosts_deny": hosts_deny_path,
                    "hosts_allow": hosts_allow_path,
                    "iptables": iptables_command,
                    "firewalld": firewalld_command,
                },
                errors,
                host,
            )
            return self.results

        issues.append({"issue": "tcp_wrapper_not_configured"})
        if not firewall_ok:
            issues.append({"issue": "firewall_not_detected"})

        limited = issues[:max_results]
        evidence = {
            "os_type": os_type,
            "config_path": self._stringify_config_path(
                {
                    "hosts_deny": hosts_deny_path,
                    "hosts_allow": hosts_allow_path,
                    "iptables": iptables_command,
                    "firewalld": firewalld_command,
                }
            ),
            "mode": self._merge_modes(
                {
                    "hosts_deny": deny_info.get("mode"),
                    "hosts_allow": allow_info.get("mode"),
                    "iptables": firewall_info.get("iptables_mode"),
                    "firewalld": firewall_info.get("firewalld_mode"),
                }
            ),
            "detected_value": limited,
            "count": len(issues),
            "checked_files": checked_count,
            "missing_files": missing_files,
            "policy": {
                "required_deny_patterns": required_deny_patterns,
                "require_allow_entries": require_allow_entries,
                "allowed_owners": allowed_owners,
                "max_mode": self._format_octal(self._parse_mode_value(max_mode)),
                "allow_group_write": allow_group_write,
                "allow_other_write": allow_other_write,
                "check_iptables": check_iptables,
                "check_firewalld": check_firewalld,
            },
            "firewall": firewall_info.get("summary"),
        }
        if host:
            evidence["host"] = host
        if errors:
            evidence["partial_errors"] = errors[:max_results]

        self.add_finding(
            vuln_id="KISA-U-28",
            title=f"{self._format_os(os_type)} 접속 IP/포트 제한 미흡",
            severity="High",
            evidence=evidence,
            tags=["KISA:U-28"],
            description="접속 제한 설정이 확인되지 않았습니다.",
            solution="hosts.deny/hosts.allow 또는 방화벽으로 접속 IP/포트를 제한하세요.",
        )
        return self.results

    def _inspect_tcp_wrapper_file(
        self,
        path: Path,
        required_patterns: Sequence[str],
        allowed_owners: Sequence[str],
        max_mode: int,
        allow_group_write: bool,
        allow_other_write: bool,
        client: Optional[SshClient],
        host: Optional[str],
        require_allow_entries: bool = False,
    ) -> Dict[str, object]:
        result = self._read_permission(path, client, host)
        info: Dict[str, object] = {
            "path": str(path),
            "mode": result.mode,
            "owner": result.owner,
            "perm": self._format_octal(result.perm),
            "raw": result.raw,
            "method": result.method,
            "issues": [],
            "status": "ok",
            "ok": False,
        }
        if result.missing:
            info["status"] = "missing"
            return info
        if result.perm is None or result.owner is None:
            info["status"] = "error"
            info["error"] = result.error
            return info

        issues = []
        allowed = {item.strip() for item in allowed_owners if item and item.strip()}
        if allowed and result.owner not in allowed:
            issues.append("owner_mismatch")
        expected_mode = self._parse_mode_value(max_mode)
        if expected_mode is not None and result.perm > expected_mode:
            issues.append("permission_too_open")
        if not allow_group_write and (result.perm & 0o020):
            issues.append("group_writable")
        if not allow_other_write and (result.perm & 0o002):
            issues.append("other_writable")

        content = self._read_config_lines(path, client, host)
        if content.lines is None:
            info["status"] = "error"
            info["error"] = content.error
            return info

        lines = [line for line in (_strip_comment(raw) for raw in content.lines) if line]
        info["line_count"] = len(lines)
        info["lines"] = lines[:20]

        if required_patterns:
            matched = []
            normalized_patterns = [self._normalize_pattern(value) for value in required_patterns]
            for line in lines:
                normalized = self._normalize_pattern(line)
                if any(normalized.startswith(pattern) for pattern in normalized_patterns if pattern):
                    matched.append(line)
            info["matched_lines"] = matched[:20]
            if not matched:
                issues.append("deny_pattern_missing")

        if require_allow_entries and not lines:
            issues.append("allow_entries_missing")

        info["issues"] = issues
        info["ok"] = not issues
        return info

    def _check_firewall(
        self,
        check_iptables: bool,
        iptables_command: str,
        check_firewalld: bool,
        firewalld_command: str,
        client: Optional[SshClient],
        host: Optional[str],
    ) -> Dict[str, object]:
        summary: Dict[str, object] = {}
        errors = []
        checked = False
        iptables_ok = False
        firewalld_ok = False
        iptables_mode = None
        firewalld_mode = None

        if check_iptables and iptables_command:
            checked = True
            result = self._run_command(iptables_command, client, host)
            iptables_mode = result.mode
            if result.lines is None:
                if result.error:
                    errors.append({"command": iptables_command, "error": result.error})
            else:
                iptables_ok = self._evaluate_iptables(result.lines)
                summary["iptables"] = {
                    "command": iptables_command,
                    "policy_detected": iptables_ok,
                }

        if check_firewalld and firewalld_command:
            checked = True
            result = self._run_command(firewalld_command, client, host)
            firewalld_mode = result.mode
            if result.lines is None:
                if result.error:
                    errors.append({"command": firewalld_command, "error": result.error})
            else:
                firewalld_ok = self._evaluate_firewalld(result.lines)
                summary["firewalld"] = {
                    "command": firewalld_command,
                    "rules_detected": firewalld_ok,
                }

        return {
            "ok": iptables_ok or firewalld_ok,
            "checked": checked,
            "errors": errors,
            "summary": summary,
            "iptables_mode": iptables_mode,
            "firewalld_mode": firewalld_mode,
        }

    def _evaluate_iptables(self, lines: Sequence[str]) -> bool:
        for raw in lines:
            line = raw.strip()
            if not line:
                continue
            lowered = line.lower()
            if lowered.startswith("chain") and "policy" in lowered:
                if "drop" in lowered or "reject" in lowered:
                    return True
            if "drop" in lowered or "reject" in lowered:
                return True
        return False

    def _evaluate_firewalld(self, lines: Sequence[str]) -> bool:
        for raw in lines:
            line = raw.strip()
            if ":" not in line:
                continue
            key, value = line.split(":", 1)
            key = key.strip().lower()
            value = value.strip()
            if key in ("services", "ports", "rich rules", "icmp-blocks", "forward-ports", "sources"):
                if value:
                    return True
        return False

    def _normalize_pattern(self, value: str) -> str:
        return re.sub(r"\s+", "", value or "").lower()

    def _get_ssh_client(self) -> Tuple[Optional[SshClient], Optional[str]]:
        connection = self.context.target.get("connection_info", {}) or {}
        credentials = self.context.target.get("credentials", {}) or {}
        host = connection.get("host") or connection.get("ip")
        user = credentials.get("username")
        key_path = credentials.get("key_path")
        password = credentials.get("password")
        proxy_jump = connection.get("proxy_jump")
        proxy_command = connection.get("proxy_command")
        identities_only = bool(connection.get("identities_only", False))

        port_raw = connection.get("port", 22)
        try:
            port = int(port_raw)
        except (TypeError, ValueError):
            raise PluginConfigError("Invalid SSH port in connection_info")

        if host and user and (key_path or password):
            return (
                SshClient(
                    host=host,
                    user=user,
                    key_path=key_path,
                    password=password,
                    port=port,
                    proxy_jump=proxy_jump,
                    proxy_command=proxy_command,
                    identities_only=identities_only,
                    sudo=bool(self.context.config.get("use_sudo", False)),
                    sudo_user=self.context.config.get("sudo_user"),
                ),
                host,
            )
        return None, host

    def _read_config_lines(
        self,
        config_path: Path,
        client: Optional[SshClient],
        host: Optional[str],
    ) -> ReadResult:
        allow_local = bool(self.context.config.get("allow_local_fallback", False))
        if client:
            try:
                command = f"cat {shlex.quote(str(config_path))}"
                result = client.run(command)
            except AdapterError as exc:
                return ReadResult(None, "remote", str(exc), host, config_path)
            if result.exit_code != 0:
                error = result.stderr.strip() or f"SSH exit code {result.exit_code}"
                return ReadResult(None, "remote", error, host, config_path)
            return ReadResult(
                result.stdout.splitlines(),
                "remote",
                None,
                host,
                config_path,
            )

        if allow_local:
            if config_path.exists():
                return ReadResult(
                    config_path.read_text().splitlines(),
                    "local",
                    None,
                    None,
                    config_path,
                )
            return ReadResult(None, "local", "File not found", None, config_path)

        return ReadResult(None, "remote", "Missing SSH credentials", host, config_path)

    def _read_permission(
        self,
        path: Path,
        client: Optional[SshClient],
        host: Optional[str],
    ) -> PermissionResult:
        allow_local = bool(self.context.config.get("allow_local_fallback", False))
        if client:
            try:
                quoted = shlex.quote(str(path))
                commands = [
                    ("stat -c '%a %U' " + quoted, "stat"),
                    ("stat -f '%Lp %Su' " + quoted, "stat"),
                    ("ls -ld " + quoted, "ls"),
                ]
                last_error = None
                for command, method in commands:
                    result = client.run(command)
                    if result.exit_code != 0:
                        error = (result.stderr or result.stdout or "").strip()
                        if _is_missing_error(error):
                            return PermissionResult(
                                None,
                                None,
                                "remote",
                                error or "File not found",
                                host,
                                path,
                                None,
                                method,
                                True,
                            )
                        last_error = error or f"SSH exit code {result.exit_code}"
                        continue
                    raw = result.stdout.strip()
                    parsed = _parse_stat_output(raw) if method == "stat" else _parse_ls_output(raw)
                    if parsed:
                        mode_value, owner = parsed
                        return PermissionResult(
                            mode_value,
                            owner,
                            "remote",
                            None,
                            host,
                            path,
                            raw,
                            method,
                        )
                    last_error = "Permission output parse failed"
                return PermissionResult(None, None, "remote", last_error, host, path)
            except AdapterError as exc:
                return PermissionResult(None, None, "remote", str(exc), host, path)

        if allow_local:
            if not path.exists():
                return PermissionResult(None, None, "local", "File not found", None, path, None, None, True)
            st = os.stat(path)
            mode_value = stat.S_IMODE(st.st_mode)
            owner = self._uid_to_name(st.st_uid)
            return PermissionResult(mode_value, owner, "local", None, None, path, None, "stat")

        return PermissionResult(None, None, "remote", "Missing SSH credentials", host, path)

    def _run_command(
        self,
        command: str,
        client: Optional[SshClient],
        host: Optional[str],
    ) -> CommandResult:
        allow_local = bool(self.context.config.get("allow_local_fallback", False))
        if client:
            try:
                result = client.run(command)
            except AdapterError as exc:
                return CommandResult(None, "remote", str(exc), host, command)
            if result.exit_code != 0:
                error = result.stderr.strip() or f"SSH exit code {result.exit_code}"
                return CommandResult(None, "remote", error, host, command)
            return CommandResult(result.stdout.splitlines(), "remote", None, host, command)

        if allow_local:
            try:
                parsed = shlex.split(command)
                result = subprocess.run(
                    parsed,
                    capture_output=True,
                    text=True,
                    timeout=60,
                    check=False,
                )
            except (OSError, subprocess.SubprocessError) as exc:
                return CommandResult(None, "local", str(exc), None, command)
            if result.returncode != 0:
                error = result.stderr.strip() or f"Command exit code {result.returncode}"
                return CommandResult(None, "local", error, None, command)
            return CommandResult(result.stdout.splitlines(), "local", None, None, command)

        return CommandResult(None, "remote", "Missing SSH credentials", host, command)

    def _add_unavailable(self, os_type: str, config_path, errors, host: Optional[str]) -> None:
        evidence = {
            "os_type": os_type,
            "config_path": self._stringify_config_path(config_path),
        }
        if host:
            evidence["host"] = host
        if errors:
            evidence["error"] = errors[0] if len(errors) == 1 else errors
        self.add_finding(
            vuln_id="KISA-U-28",
            title=f"{self._format_os(os_type)} 접속 제한 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-28"],
            description="접속 제한 설정을 확인할 수 없습니다.",
            solution="대상 접근 권한과 관련 파일/명령 실행 권한을 확인하세요.",
        )

    def _uid_to_name(self, uid: int) -> str:
        try:
            return pwd.getpwuid(uid).pw_name
        except KeyError:
            return str(uid)

    def _to_positive_int(self, value: object, name: str) -> int:
        try:
            parsed = int(value)
        except (TypeError, ValueError):
            raise PluginConfigError(f"{name} must be an integer")
        if parsed <= 0:
            raise PluginConfigError(f"{name} must be > 0")
        return parsed

    def _parse_mode_value(self, value: int) -> Optional[int]:
        if value < 0:
            return None
        if value > 0o777:
            value = int(str(value), 8)
        if value <= 0o777:
            return value
        return None

    def _merge_modes(self, modes: Dict[str, Optional[str]]):
        clean = {key: value for key, value in modes.items() if value}
        if not clean:
            return None
        unique = set(clean.values())
        if len(unique) == 1:
            return next(iter(unique))
        return clean

    def _format_octal(self, value: Optional[int]) -> Optional[str]:
        if value is None:
            return None
        return format(value, "04o")

    def _stringify_config_path(self, value):
        if isinstance(value, Path):
            return str(value)
        if isinstance(value, list):
            return [self._stringify_config_path(item) for item in value]
        if isinstance(value, dict):
            return {key: self._stringify_config_path(val) for key, val in value.items()}
        return value

    def _format_os(self, os_type: str) -> str:
        display = {
            "linux": "Linux",
            "solaris": "Solaris",
            "aix": "AIX",
            "hpux": "HP-UX",
        }
        return display.get(os_type, os_type.upper())
