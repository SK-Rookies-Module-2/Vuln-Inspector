"""Remote plugin for KISA U-29 hosts.lpd permission checks."""
...
DEFAULT_HOSTS_LPD_PATH = "/etc/hosts.lpd"
...
class HostsLpdPermissionCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        hosts_lpd_path = Path(
            self.context.config.get("hosts_lpd_path") or DEFAULT_HOSTS_LPD_PATH
        )
        ...
        result = self._read_permission(hosts_lpd_path)
        if result.missing:
            return self.results
        if result.perm is None or result.owner is None:
            self._add_unavailable(os_type, hosts_lpd_path, result)
            return self.results
        ...
        if issues:
            evidence = self._base_evidence(os_type, hosts_lpd_path, result)
            ...
            self.add_finding(
                vuln_id="KISA-U-29",
                title=f"{self._format_os(os_type)} /etc/hosts.lpd 권한 설정 미흡",
                severity="Low",
                evidence=evidence,
                tags=["KISA:U-29"],
                description="hosts.lpd 파일 소유자 또는 권한 설정이 기준을 만족하지 않습니다.",
                solution="파일 소유자를 root로 설정하고 권한을 600 이하로 제한하세요.",
            )
    ...
    def _add_unavailable(self, os_type: str, path: Path, result: PermissionResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-29",
            title=f"{self._format_os(os_type)} /etc/hosts.lpd 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-29"],
            description="/etc/hosts.lpd 권한 정보를 확인할 수 없습니다.",
