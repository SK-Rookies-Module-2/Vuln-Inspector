"""Remote plugin for KISA U-63 sudoers permission checks."""
...
DEFAULT_SUDOERS_PATH = "/etc/sudoers"
...
class SudoersPermissionCheck(BasePlugin):
    def check(self) -> List[Finding]:
        ...
        sudoers_path = Path(self.context.config.get("sudoers_path") or DEFAULT_SUDOERS_PATH)
        ...
        result = self._read_permission(sudoers_path)
        if result.missing:
            self._add_unavailable(os_type, sudoers_path, result)
            return self.results
        ...
        if issues:
            evidence = self._base_evidence(os_type, sudoers_path, result)
            ...
            self.add_finding(
                vuln_id="KISA-U-63",
                title=f"{self._format_os(os_type)} /etc/sudoers 권한 설정 미흡",
                severity="Medium",
                evidence=evidence,
                tags=["KISA:U-63"],
                description="sudoers 파일 소유자 또는 권한 설정이 기준을 만족하지 않습니다.",
                solution="sudoers 파일 소유자를 root로 설정하고 권한을 640 이하로 제한하세요.",
            )
    ...
    def _add_unavailable(self, os_type: str, path: Path, result: PermissionResult) -> None:
        ...
        self.add_finding(
            vuln_id="KISA-U-63",
            title=f"{self._format_os(os_type)} /etc/sudoers 점검 불가",
            severity="Info",
            evidence=evidence,
            tags=["KISA:U-63"],
            description="sudoers 권한 정보를 확인할 수 없습니다.",
