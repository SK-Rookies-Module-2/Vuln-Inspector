...
• 플러그인 위치(권장): `plugins/remote/kisa_u45/`

### plugin.yml 설계(예시)
```yaml
id: "remote_kisa_u45"
name: "Mail Service Version Check"
version: "0.1.0"
type: "remote"
category: "infrastructure"
tags:
  - "KISA:U-45"
description: "Check sendmail/postfix version information."
config_schema:
  required:
    - os_type
  properties:
    ...
    sendmail_command:
      type: string
      default: "sh -lc 'sendmail -d0.1 < /dev/null | grep Version'"
    postfix_command:
      type: string
      default: "postconf mail_version"
    ...
    allow_local_fallback:
      ...
      default: false
entry_point: "main.py"
class_name: "MailServiceVersionCheck"
```
...
2) 입력값 정리  
• `os_type` 필수. `sendmail_command`/`postfix_command` 기본값 제공.  
• `min_sendmail_version`/`min_postfix_version`가 있으면 버전 비교 수행.  
• `report_unknown=true`이면 기준값이 없는 경우 Info로 보고.  
• 원격 접근 정보가 없고 `allow_local_fallback=true`이면 로컬 명령 실행 허용.
3) 명령 실행  
• Sendmail/Postfix 버전 명령 실행 후 버전 문자열 파싱.  
• 명령이 없으면 해당 서비스 미사용으로 간주.  
• 실패 시 `partial_errors`로 기록.
4) 판정  
• 버전이 기준값 미만이면 취약.  
...
5) 결과 기록  
• 취약 시 `add_finding()` 호출:  
  - `vuln_id="KISA-U-45"`, `severity="High"`  
  - `tags=["KISA:U-45"]`  
  - `evidence`: `os_type`, `config_path`, `detected_value`, `host`, `partial_errors`  
• 기준 미설정 시 `Info` Finding 기록(선택).

### 파서 설계(요약)
• Sendmail: `Version x.y.z` 문자열 파싱.  
• Postfix: `mail_version = x.y.z` 문자열 파싱.  
• 버전 비교는 숫자 토큰 기반 비교.

### 테스트 계획
• 유닛:  
  - Sendmail/Postfix 버전 파서 테스트.  
  - 버전 비교 로직 테스트.  
  - 명령 미존재/오류 처리 테스트.
...
curl -X POST http://127.0.0.1:8000/api/v1/jobs \
  ...
  -d '{
    "target_id": 1,
    "scan_scope": ["remote_kisa_u45"],
    "scan_config": {
      "remote_kisa_u45": {
        "os_type": "linux",
        "min_sendmail_version": "8.17.1",
        "min_postfix_version": "3.6.0",
        "use_sudo": true
      }
    }
  }'
