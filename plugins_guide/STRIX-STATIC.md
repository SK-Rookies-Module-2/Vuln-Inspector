STRIX-STATIC: 외부 정적(Strix) 보안 스캔
• 중요도: 해당 없음(외부 스캐너 결과에 따름)
• 점검 목적: Strix CLI를 이용해 GIT_REPO 대상의 정적 취약점 탐지 결과를 표준 Finding으로 변환한다.
• 보안 위협: 공급망 취약점, 취약 라이브러리, 코드 기반 취약 패턴 등.

점검 대상 및 판단 기준
• 대상: GIT_REPO (repo_url 또는 repo_path)
...
• 채널: static (코드/리포지토리 기반)
• 대상: TargetType.GIT_REPO
• 플러그인 위치: `plugins/static/strix_scan/`
• 플러그인 ID: `static_strix_scan`
...
• instruction/instruction_file로 분석 범위, 제외 경로, 정책 지시 가능  
• non_interactive 기본 true(`-n`)로 TUI 대신 로그 기반 실행

### 점검 흐름

1. Target 검증  
   • `context.target.type`이 GIT_REPO가 아니면 오류.
2. 실행 준비  
   • `job_id` 기반 run_name 생성(없으면 기본값).
   • workdir: `storage/artifacts/{job_id}/strix/{run_name}/`
3. Strix 실행  
   • `strix -n --target <repo_url|repo_path> --scan-mode <mode> --instruction ... --run-name <run_name>`
4. 결과 수집  
...
{
  "target_id": 1,
  "scan_scope": ["static_strix_scan"],
  "scan_config": {
    "static_strix_scan": {
      "scan_mode": "deep",
      "instruction": "Focus on dependency risks in requirements.txt",
      "run_name": "job-1-strix-static"
    }
  }
}
```
