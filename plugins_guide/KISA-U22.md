...
• 플러그인 위치(권장): `plugins/remote/kisa_u22/`

### plugin.yml 설계(예시)
```yaml
id: "remote_kisa_u22"
name: "Services File Permission Check"
version: "0.1.0"
type: "remote"
category: "infrastructure"
tags:
  - "KISA:U-22"
description: "Check /etc/services ownership and permissions."
config_schema:
  required:
    - os_type
  properties:
    ...
    allow_local_fallback:
      ...
      default: false
entry_point: "main.py"
class_name: "ServicesFilePermissionCheck"
```

### OS별 기본 경로(코드 내 기본값)
• 공통: `services_path=/etc/services`

### 점검 흐름
1) Target 검증  
• `context.target.type`이 SERVER가 아니면 `PluginConfigError`로 종료.
2) 입력값 정리  
• `os_type` 필수. `services_path` 오버라이드는 있으면 사용, 없으면 기본값 적용.  
• `allowed_owners` 기본값 root/bin/sys, `max_mode` 기본값 644.  
...
5) 결과 기록  
• 취약 시 `add_finding()` 호출:  
  - `vuln_id="KISA-U-22"`, `severity="High"`  
  - `title`에 OS 포함(예: "Linux /etc/services 권한 설정 미흡")  
  - `tags=["KISA:U-22"]`  
...
• 유닛:  
  ...
  - owner 허용 목록/권한 비교/쓰기 권한 판정 테스트.  
