...
- sudo가 필요하면 sudo_user와 use_sudo 설정
...
{
  "target_id": 1,
  "scan_scope": ["remote_linux_kisa_u02"],
  "scan_config": {}
}
```
...
`~/.ssh/bastion-server-key.pem`을 스캐너 머신에 저장하고 권한 설정:
...
curl -X POST http://127.0.0.1:8000/api/v1/targets \
  ...
  -d '{
    ...
    "description": "AWS private subnet Ubuntu via bastion (U-02)"
  }'
```
